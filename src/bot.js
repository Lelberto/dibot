	...
	constructor(config = utils.defaultConfig) {
		...
		this._client.on('message', (msg) => {
			if (msg.content.startsWith(`!${this._config.command}`)) { // Si c'est une commande
				...
				!this._guildsData.disabledStatuses.has(msg.guild.id) && this._processMessage(msg);
			}
		});
	}
	...
	_processCommand(msg) {
		...
		switch (args[0]) {
			...
			case 'status':
				switch (args[1]) {
					default:
						channel.send(`État du BOT sur ce serveur : ${!this._guildsData.disabledStatuses.has(guild.id) ? ':white_check_mark: activé' : ':x: désactivé'}`);
						break;
					case 'on':
					case 'enable':
					case 'enabled':
						this._guildsData.disabledStatuses.delete(guild.id);
						channel.send('BOT activé sur ce serveur');
						break;
					case 'off':
					case 'disable':
					case 'disabled':
						this._guildsData.disabledStatuses.add(guild.id);
			...
			case 'tts':
				switch (args[1]) {
					default:
						channel.send(`Messages TTS pour le BOT sur ce serveur : ${this._guildsData.enabledTTS.has(guild.id) ? ':white_check_mark: activés' : ':x: désactivés'}`);
						break;
					case 'on':
					case 'enable':
					case 'enabled':
						this._guildsData.enabledTTS.add(guild.id);
						channel.send('Messages TTS pour le BOT activés sur ce serveur');
						break;
					case 'off':
					case 'disable':
					case 'disabled':
						this._guildsData.enabledTTS.delete(guild.id);
	...
	_processMessage(msg) {
		...
		for (let i = 0; i < triggers.length; i++) {
			if (i === this._config.max_trigger_count) {
				...
				maxTriggerMessages.length > 0 && channel.send(maxTriggerMessages[_.random(0, maxTriggerMessages.length - 1)], { tts: this._guildsData.enabledTTS.has(guild.id) });
				break;
			}
			...
			reply !== '' && channel.send(reply, { tts: this._guildsData.enabledTTS.has(guild.id) });
		}
	}
